...
class WordInfo:
    ...
    def __init__(self, wotd:str) -> None:
        ...
        def_json = json.loads(get(f'https://wordsapiv1.p.rapidapi.com/words/{wotd}/definitions', headers=api_headers).text)
        
        # get json data for the frequency 
        freq_json = json.loads(get(f'https://wordsapiv1.p.rapidapi.com/words/{wotd}/frequency', headers=api_headers).text)

        # get the json data for examples
        exam_json = json.loads(get(f'https://wordsapiv1.p.rapidapi.com/words/{wotd}/examples', headers=api_headers).text)
